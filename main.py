...
# IGRAČ
playerSL = pygame.image.load('player.png')  # učitava sliku igrača
player_sudar = pygame.image.load('player_sudar.png')  # učitava sliku igrača tijekom sudara
s_player1 = 100
v_player1 = 64
playerSL = pygame.transform.scale(playerSL, (s_player1, v_player1))  # oblikuje dimenzije slike
player_sudar = pygame.transform.scale(player_sudar, (s_player1, v_player1))  # oblikuje dimenzije slike
playerX = 500
playerY = 400
playerX_potez = 0  # definira pomak igrača na X osi
playerY_potez = 0  # definira pomak igrača na Y osi

# OBJEKTI
oilSL = pygame.image.load('oil.png')  # učitava sliku mrlje ulja
s_oil = 60
v_oil = 60
oilSL = pygame.transform.scale(oilSL, (s_oil, v_oil))  # određuje dimenzije slike
oilX = s_prozor
oilY = random.randint(130, (v_prozor - v_oil - 130))  # random koordinate
oilX_potez = 0.3  # pomak objekta na X osi

autoSL = pygame.image.load('player2rot.png')  # učitava sliku automobila
s_auto = 100
v_auto = 64
autoSL = pygame.transform.scale(autoSL, (s_auto, v_auto))
autoX = s_prozor
autoY = random.randint(130, (v_prozor - v_auto - 130))  # random koordinate
autoX_potez = 0.55  # pomak
...
while uvjet:
    ...
    auto(autoX, autoY)  # poziva funkciju auto koja crta sliku automobila
    autoX -= autoX_potez  # mijenja koordinate za pomicanje ulijevo
    if autoX < -s_auto:  # provjerava je li slika izašla iz okvira, te ako je stvara novu
        autoX = s_prozor
